...
st.markdown("""
<div class="hero">
  <div class="hero-title">💧 The Hidden Water Map</div>
  <div class="hero-subtitle">Where today's food production depends on water systems under structural pressure</div>
  <span class="hero-tag">FAOSTAT & FAO AQUASTAT · 2022 analysis</span>
</div>
""", unsafe_allow_html=True)
...
kpi_data = [
    ("💧", "Severe water stress", "≥50%",
     "Project threshold — stricter than the SDG 6.4.2 25% standard", "kpi-blue"),
    ("🌱", "Highest crop exposure", f"{sugar_exposure:.1f}%",
     "Sugar cane — 2022 production in countries with water stress ≥50%", "kpi-green"),
    ("⚠️", "High-pressure countries", f"{n_hotspot_countries}",
     "Countries meeting both project hotspot thresholds", "kpi-orange"),
    ("◉", "Data coverage", f"{coverage_min:.0f}%–{coverage_max:.0f}%",
     "Varies by crop — see Methodology", "kpi-purple"),
]

for col, (icon, label, value, caption, card_class) in zip(kpi_cols, kpi_data):
    with col:
        st.markdown(f"""
...
<div class="insight-card">
  ...
  <div class="insight-text">
    ...
    Water stress can be a resilience signal when substantial food production depends
    on heavily pressured water systems.
  </div>
</div>
""", unsafe_allow_html=True)
