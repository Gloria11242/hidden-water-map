...
.hero-image {
    ...
    border-radius:20px;
    background:
      linear-gradient(90deg, rgba(8,39,58,.88) 0%, rgba(8,39,58,.63) 42%, rgba(8,39,58,.22) 100%),
      url("https://images.unsplash.com/photo-1464226184884-fa280b87c399?auto=format&fit=crop&w=1800&q=85") center/cover no-repeat;
    padding:34px 38px;
    color:white;
...
.hero-image:after {
    content:"";
    position:absolute; right:-90px; top:-120px; width:420px; height:420px;
    border-radius:50%;
...
.hero-kicker {font-size:.78rem;font-weight:800;letter-spacing:1.5px;text-transform:uppercase;opacity:.88}
.hero-big {font-size:2.65rem;font-weight:850;line-height:1.05;margin-top:7px;letter-spacing:-1px}
.hero-small {font-size:1.04rem;max-width:620px;margin-top:10px;line-height:1.45;opacity:.96}
.hero-quote {
    position:absolute; right:28px; bottom:22px; width:300px;
    background:rgba(8,34,48,.82); border-left:4px solid #7ee0b5;
    border-radius:12px; padding:14px 17px; font-style:italic; font-size:.88rem;
    line-height:1.4; z-index:3;
}
.hero-quote-strip {
    margin-top:10px; margin-bottom:18px;
    background:#eaf6fb; border:1px solid #d4e8f1;
    border-left:5px solid #58a9c9; border-radius:12px;
    padding:11px 16px; color:#294f67; font-size:.9rem;
    box-shadow:0 2px 8px rgba(18,48,74,.04);
}
...
.card-blue,.card-red,.card-green,.card-orange,.card-purple{background:linear-gradient(145deg,#eaf5ff,#f7fbff)}
...
@media(max-width:1000px){.card-row{grid-template-columns:repeat(2,1fr)}.hero-content{max-width:100%}.hero-quote{position:static;width:auto;margin-top:18px}.hero-image{min-height:360px}}
...
<div style="padding:8px 6px 16px 6px;">
  <div style="font-size:1.15rem;font-weight:850;color:#12304a;line-height:1.05;">💧 The Hidden<br>Water Map</div>
  <div style="font-size:.65rem;color:#6b7f8f;margin-top:5px;letter-spacing:.7px;font-weight:700;">FOOD TODAY · WATER TOMORROW</div>
</div>
<div style="height:1px;background:#dce6ed;margin-bottom:14px;"></div>
""", unsafe_allow_html=True)
...
<div class="hero-quote-strip">💧 <b>Core message:</b> High production does not necessarily mean a resilient food system — current output can coexist with substantial water-system pressure.</div>
""", unsafe_allow_html=True)

st.markdown(
    f'<div class="dashboard-meta"><span>FAOSTAT & AQUASTAT · 2022</span><span>Countries: {dashboard["Country"].nunique()}</span><span>Crops: {dashboard["Crop"].nunique()}</span></div>',
    unsafe_allow_html=True
)
