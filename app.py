...
.brand-block{display:flex;align-items:center;gap:10px;padding:4px 4px 18px;border-bottom:1px solid #dfe8ef;margin-bottom:14px}
.brand-mark{width:38px;height:38px;border-radius:12px;background:linear-gradient(145deg,#dff5ea,#dbeeff);display:flex;align-items:center;justify-content:center;font-size:1.25rem}
.brand-title{font-size:1.05rem;line-height:1.0;font-weight:850;color:#12304a}
.brand-sub{font-size:.46rem;letter-spacing:.65px;color:#60798a;margin-top:5px;font-weight:750}
.nav-list{margin:0 0 17px}
.nav-item{padding:8px 10px;border-radius:9px;color:#29445a;font-size:.76rem;font-weight:650;margin:2px 0}
.nav-item.active{background:#dcecff;color:#1761bd;font-weight:800}
.nav-item:hover{background:#edf4fb}
[data-testid="stSidebar"]{box-shadow:2px 0 12px rgba(18,48,74,.03)}
.dashboard-meta {
...
.hero-image {
    ...
    border-radius:20px;
    background:
      linear-gradient(90deg, rgba(9,39,60,.86) 0%, rgba(9,39,60,.58) 42%, rgba(9,39,60,.18) 100%),
      url("https://images.unsplash.com/photo-1785905552848-61a19196160b?auto=format&fit=crop&fm=jpg&ixlib=rb-4.1.0&q=80&w=2200") center/cover no-repeat;
    padding:34px 38px;
    color:white;
...
.hero-quote {
    position:absolute; right:26px; bottom:22px; width:330px;
...
<div class="brand-block">
  ...
  <div>
    <div class="brand-title">The Hidden<br>Water Map</div>
    <div class="brand-sub">FOOD TODAY · WATER TOMORROW</div>
  </div>
</div>
<div class="nav-list">
  <div class="nav-item active">⌂ &nbsp; Overview</div>
  <div class="nav-item">◉ &nbsp; Pressure Map</div>
  <div class="nav-item">♧ &nbsp; Crop Exposure</div>
  <div class="nav-item">▥ &nbsp; Hotspot Analysis</div>
  <div class="nav-item">● &nbsp; Country Insights</div>
  <div class="nav-item">♟ &nbsp; India & Pakistan</div>
...
st.markdown(
    f'<div class="dashboard-meta"><span>FAOSTAT & AQUASTAT · 2022</span><span>Countries: {n_countries}</span><span>Crops: {len(CROPS)}</span></div>',
    unsafe_allow_html=True
)
st.caption("Hero image: irrigation field via Unsplash. Dashboard analysis: FAOSTAT + FAO AQUASTAT.")
